// nopt.cu - Matrix multiplication CUDA (no shared memory)
//
// Compile:
//   nvcc nopt.cu -o nopt
// Run:
...
//   contoh: ./nopt 1024 16
//
// Menghitung C = A x B, row-major. Ada timing total & compute.
...
// Kernel naive: satu thread menghitung satu elemen C[row,col]
...
int main(int argc, char **argv) {
    ...
    printf("CUDA matmul (no shared memory), N = %d, blockSize = %d\n", N, blockSize);
    ...
    matrixMulKernel<<<grid, threads>>>(d_A, d_B, d_C, N);
    ...
    // Verifikasi terhadap CPU untuk N kecil (biar tidak terlalu lama)
    if (N <= 512) {
