#!/bin/bash
#SBATCH --job-name=gs-cluster
#SBATCH --partition=batch
#SBATCH --nodes=1-8        # Request up to 8 nodes
#SBATCH --output=gs_cluster_%j.out
#SBATCH --error=gs_cluster_%j.err
...
RESULTS_DIR="gauss_seidel_results_cluster"
mkdir -p "$RESULTS_DIR"

# Create CSV file for results
RESULTS_CSV="$RESULTS_DIR/gauss_seidel_results_cluster.csv"
echo "N,np,ComputeTime,CommunicationTime,TotalTime,Iterations" > "$RESULTS_CSV"

# Summary file
SUMMARY_FILE="$RESULTS_DIR/summary.txt"
echo "Gauss-Seidel Iteration - Cluster Environment Results" > "$SUMMARY_FILE"
echo "=============================================" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
...
for N in "${SIZES[@]}"; do
    echo "====================================================="
    echo "Running Gauss-Seidel solver on cluster with N=$N"
    echo "====================================================="
    ...
    for np in "${PROCS[@]}"; do
        ...
        result=$(run_with_timeout "mpirun --mca btl_tcp_if_exclude docker0,lo -np $np ./gauss_seidel_solver $N")
