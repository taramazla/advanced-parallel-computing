...
do
    echo "Compiling matrix_matrix_${size}.c"
    # First copy the base code
    cp matrix_matrix_256.c matrix_matrix_${size}.c

    # Replace the matrix size in the copied file
    sed -i.bak "s/#define N 256/#define N ${size}/" matrix_matrix_${size}.c
    rm -f matrix_matrix_${size}.c.bak

    # Compile with MPI
    mpicc -o matrix_matrix_${size}.o matrix_matrix_${size}.c

    echo "Compiled matrix_matrix_${size}.o"
    echo "-----------------------------------"
done
...
mpirun --mca btl_tcp_if_exclude docker0,lo -np ${procs} ./matrix_matrix_${size}.o
EOF
