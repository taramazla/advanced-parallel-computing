...
do
    echo "Compiling matrix_matrix_cannon_${size}.c"
    # First copy the base code
    cp matrix_matrix_cannon_256.c matrix_matrix_cannon_${size}.c

    # Replace the matrix size in the copied file
    sed -i.bak "s/#define N 256/#define N ${size}/" matrix_matrix_cannon_${size}.c
    rm -f matrix_matrix_cannon_${size}.c.bak

    # Compile with MPI
    mpicc -O3 -std=c11 -DN="${size}" -o matrix_matrix_cannon_${size}.o matrix_matrix_cannon_${size}.c -lm

    echo "Compiled matrix_matrix_cannon_${size}.o"
    echo "-----------------------------------"
done
...
mpirun --mca btl_tcp_if_exclude docker0,lo -np ${procs} ./matrix_matrix_cannon_${size}.o
EOF
