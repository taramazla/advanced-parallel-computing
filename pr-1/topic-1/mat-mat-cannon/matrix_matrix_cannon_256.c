...
int main(int argc, char **argv) {
    ...
    // --- Cannon's initial alignment (skewing) ---
    // Shift A left by row coordinate, B up by column coordinate
    MPI_Status st;
    int src, dst;

    double t0_align = MPI_Wtime();

    // A: shift along columns (dim=1), disp = -coords[0]
    MPI_Cart_shift(grid, /*dim=*/1, /*disp=*/-coords[0], &src, &dst);
    MPI_Sendrecv_replace(Ablock, bs*bs, MPI_DOUBLE, dst, 100, src, 100, grid, &st);

    // B: shift along rows (dim=0), disp = -coords[1]
    MPI_Cart_shift(grid, /*dim=*/0, /*disp=*/-coords[1], &src, &dst);
    MPI_Sendrecv_replace(Bblock, bs*bs, MPI_DOUBLE, dst, 200, src, 200, grid, &st);

    t_comm += MPI_Wtime() - t0_align;

    // --- Main Cannon loop: q steps ---
    double comp_time_local = 0.0;
    for (int step = 0; step < q; step++) {
        // Compute Cblock += Ablock * Bblock
        double t0_comp = MPI_Wtime();
        dgemm_block(bs, Ablock, Bblock, Cblock);
        comp_time_local += (MPI_Wtime() - t0_comp);

        // Rotate A left by 1 (dim=1, disp=-1) and B up by 1 (dim=0, disp=-1)
        double t0_shift = MPI_Wtime();
        MPI_Cart_shift(grid, 1, -1, &src, &dst);
        MPI_Sendrecv_replace(Ablock, bs*bs, MPI_DOUBLE, dst, 300, src, 300, grid, &st);

        MPI_Cart_shift(grid, 0, -1, &src, &dst);
        MPI_Sendrecv_replace(Bblock, bs*bs, MPI_DOUBLE, dst, 400, src, 400, grid, &st);
